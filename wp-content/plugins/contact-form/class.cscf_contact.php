...
class cscf_Contact
{
    ...
    function IsValid()
    {
        ...
        if (cscf_PluginSettings::ConfirmEmail()) {
            if ($this->Email != $this->ConfirmEmail) $this->Errors['confirm-email'] = __('Sorry the email addresses do not match.', 'contactform');
        }

        //email

        if (strlen($this->Email) == 0) $this->Errors['email'] = __('Please give your email address.', 'contactform');

        //confirm email
        if (cscf_PluginSettings::ConfirmEmail()) {
            if (strlen($this->ConfirmEmail) == 0) $this->Errors['confirm-email'] = __('Please confirm your email address.', 'contactform');
        }

        //name

        if (strlen($this->Name) == 0) $this->Errors['name'] = __('Please give your name.', 'contactform');

        //message

        if (strlen($this->Message) == 0) $this->Errors['message'] = __('Please enter a message.', 'contactform');

        //email invalid address

        if (strlen($this->Email) > 0 && !filter_var($this->Email, FILTER_VALIDATE_EMAIL)) $this->Errors['email'] = __('Please enter a valid email address.', 'contactform');
        ...
        if ($this->RecaptchaPublicKey <> '' && $this->RecaptchaPrivateKey <> '') {
            ...
            if (!$resp->is_valid) $this->Errors['recaptcha'] = __('Sorry the code wasn\'t entered correctly please try again.', 'contactform');
        }
    ...
    function SendMail()
    {
        ...
        if ($this->EmailToSender) {
            ...
            $message .= __("Here is a copy of your message :", "contactform") . "\n\n";
